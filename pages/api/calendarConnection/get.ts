import { _getCalendarConnection } from './_operations'
...
const post = async (req, res) => {
  ...
  try {
    const calendarConnection = await _getCalendarConnection(findOneArgs)

    return res.status(200).json({ data: calendarConnection })
  } catch (err) {
