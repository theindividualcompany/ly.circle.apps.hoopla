...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const body = toUrlEncoded({
    ...
    code,
    scope: scopes.join(" "),
    redirect_uri: hostname + "/api/integrations/office365calendar/callback",
  ...
  const encrypted = symmetricEncrypt(JSON.stringify(responseBody), process.env.ENCRYPTION_KEY)
